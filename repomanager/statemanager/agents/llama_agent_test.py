...
print(os.getcwd())
print(sys.path)
from agents.llama_agents.FunctionCaller import GodFather, AGENT_INSTRUCTIONS, get_context, DEFAULT_MODEL_NAME, godfather_tools
...
if __name__ == "__main__":
    ...
    gf = GodFather.from_tools(
        tools=godfather_tools,
        verbose=True,
        # max_function_calls=5,
        llm=OpenAI('gpt-4'),
    )

    print(gf.decision("""Hi"""))
    
    # chat_history = [ChatMessage(role="system", content=AGENT_INSTRUCTIONS)]

    # while True:

    # _input = """What is a fusion retriever, and how to use it?"""
